...
This is a consolidated repository for Lenny's Podcast Recapper, containing 303+ episode transcripts from Lenny's Podcast in two formats (structured markdown and raw text), with an AI-generated topic index for easy discovery.
...
│   ├── raw-transcripts/
│   │   └── {Guest Name}.txt     # Raw speaker-timestamped transcripts (311 files)
│   └── index/
...
### Raw (data/raw-transcripts/)
Plain text files with speaker-timestamped dialogue. These cover 311 episodes (8 more than the structured set) and serve as a fallback or alternate format.

## Index

The `data/index/` folder contains AI-generated keyword tags for each episode:
...
Grep pattern="product.market fit" path="data/episodes/"

# Search with context lines for better understanding
Grep pattern="early stage" path="data/episodes/" output_mode="content" -C=5
```

### 2. Read frontmatter first (lines 1-15)
Get metadata before deciding to read more:
```
Read file_path="data/episodes/guest-name/transcript.md" limit=15
```
